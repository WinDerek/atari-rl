    ...
    for episode_index in range(num_episodes):
        ...
        while True:
            ...
            if done:
                ...
                episode_reward_array[episode_index] = episode_reward

                break
    print('Average episode reward: %.2f' % (sum(episode_reward_array) / episode_reward_array.shape[0]))

    # Persist the results
    results_path = Path("./results/dqn")
    results_path.mkdir(parents=True, exist_ok=True)
    results_file = results_path.joinpath("results.pkl")
    results = { "episode_reward_array": episode_reward_array }
